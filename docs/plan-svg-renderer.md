...
- [ ] `qrgen/render_svg.go` with `renderSVG(m *matrix, opts renderOptions) ([]byte, error)`: apply `withDefaults`, guard nil matrix and invalid side, emit the XML declaration, the `<svg>` root with module-unit `viewBox` and pixel `width`/`height`, a full-canvas background `<rect>`, and a single foreground `<path>` whose `d` draws each dark module as `M (c+qz) (r+qz) h1 v1 h-1 z` (one unit square per dark module) offset by the quiet zone.
- [ ] A `colorToHex(c color.Color) (hex string, opacity float64)` helper that converts to `#RRGGBB` and returns fractional opacity when alpha < 0xFFFF.
- [ ] Emit `fill-opacity` attributes only when the corresponding colour is non-opaque, so the common black-on-white case stays minimal.
- [ ] Tests in `qrgen/render_svg_test.go`: output parses as XML (`encoding/xml` round-trip into a generic struct), the `viewBox` and `width`/`height` match the option math, the background rect colour matches `WithColors`, the foreground path is present, and the count of move commands (`M`) in the path equals the number of dark modules in the matrix. Cover default options, custom module size / quiet zone, and a custom colour pair including an alpha-bearing foreground.
