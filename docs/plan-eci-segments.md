...
5. **Spec-faithful placement.** The ECI segment (mode indicator `0111` followed by the designator) is emitted once at the head of the data, before the mixed-mode segment sequence, per ISO/IEC 18004. It changes how byte segments are interpreted; it does not alter numeric/alphanumeric decoding.
