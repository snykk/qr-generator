...
- [x] Helper `otsuBinariseFromGray` di-ekstrak supaya dispatch dapat menjalankan Otsu dan Sauvola di atas pass grayscale yang sama tanpa walk image ulang. Helper `foregroundRatio` yang awalnya juga di-ekstrak dipensiunkan di T5 setelah benchmark menunjukkan post-check O(width * height)-nya memakan ~3–5% di Otsu happy path tanpa manfaat perilaku, karena kegagalan `findFinders` sudah secara ketat menutupi kasus "rasio tidak sehat".
...
- [x] Menjalankan ulang benchmark image-stage yang ada (`Small`, `MultiBlock`, `URL`, `FromPNGDecode`) di master dan di branch pada dispatch v3 (Apple M5, `-count=5`, `-benchtime=1s`). Cut pertama membuat dispatch menjalankan pass `foregroundRatio` penuh O(width * height) di setiap decode, yang memakan ~3–5% di Otsu happy path. Helper tersebut dan konstanta `foregroundLo`/`foregroundHi` terkait dipensiunkan setelah analisis satu baris: binarisasi single-class yang degenerate tidak akan pernah menghasilkan triple 1:1:3:1:1 yang valid, sehingga kegagalan `findFinders` sudah secara ketat menutupi kasus "rasio tidak sehat". Median akhir (branch vs master, +% = branch lebih lambat): `Small` 651/648 ns/op = +0.5%, `MultiBlock` 1308/1318 ns/op = -0.8%, `URL` 1085/1068 ns/op = +1.6%, `FromPNGDecode` 564/555 ns/op = +1.6%. Semuanya dalam variansi run-to-run milik master sendiri.
- [x] Menambah `BenchmarkDecodeImageSauvolaFallback` yang memaksa jalur reaktif via fixture berbentuk `TestT4ConstantQuietZoneDarkening` (QR tidak disentuh, quiet zone abu-abu uniform di 70). Pada hardware yang sama nilainya mendarat di ~1074 ns/op dengan 1.07 MB/op dan 241 allocs/op — sekitar 1.9x Otsu fast path pada V1 karena dispatch menjalankan binarisasi Otsu, finder detection, lalu pass Sauvola penuh dan finder detection kedua. Lompatan alokasi datang dari kedua integral image berbasis `uint64` berukuran `(w+1) * (h+1)`.
