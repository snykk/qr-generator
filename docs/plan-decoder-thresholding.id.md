...
- [ ] **Pre-check (proaktif, gratis):** selama Otsu berjalan, simpan nilai maksimum between-class variance `σ²_B` dan total variance `σ²_T` yang sudah dihitung dari histogram. Rasionya `η = σ²_B / σ²_T` adalah ukuran separability standar yang hidup di `[0, 1]`; nilai dekat 1 menandakan histogram bimodal yang terpisah baik, nilai dekat 0 menandakan unimodal. Kalau `η < η_min`, lewati langkah binarisasi Otsu sepenuhnya dan langsung ke Sauvola — ini menghemat satu pass finder detection penuh di kasus kegagalan.
- [ ] **Post-check (reaktif, defense-in-depth):** ketika binarisasi Otsu memang dijalankan, anggap outputnya tidak sehat kalau salah satu dari: (a) `findFinders` mengembalikan kurang dari tiga kandidat valid, atau (b) rasio foreground dari image yang sudah dibinarisasi jatuh di luar `[0.05, 0.95]` (output single-class yang degenerate). Pada kedua kasus tersebut, re-binarise image grayscale dengan Sauvola lalu jalankan ulang finder detection. Baru kembalikan `ErrFinderNotFound` kalau pass Sauvola juga gagal.
- [ ] Default `η_min = 0.5` sebagai threshold awal mengikuti paper asli Otsu; perlakukan nilai eksak ini sebagai knob tuning yang akan dikunci setelah fixture sintetis T4 mendarat (lihat Open Questions).
- [ ] Tambahkan sentinel debug var internal (variabel `binariserUsed` yang tidak di-export, di-set di tes via hook tanpa build tag) supaya kita dapat assert path mana yang dijalankan di tes tanpa membocorkan ke API publik. Hook melacak tiga state: `binariserOtsu`, `binariserSauvolaProactive` (skip Otsu via η), `binariserSauvolaReactive` (fall back setelah Otsu).
- [ ] Tes: round-trip Decode end-to-end pada image gradient sintetis yang gagal dengan Otsu saja dan berhasil dengan fallback; assert cabang proaktif dijalankan pada image dengan η rendah (gradient), cabang reaktif dijalankan pada image dengan η tinggi yang threshold Otsu-nya tetap jatuh di tempat yang salah, dan cabang Otsu-only dijalankan pada PNG V1 bersih.
