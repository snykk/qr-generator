...
**Stage 2 — post-check reaktif.** Kalau `η ≥ η_min` kita jalankan binarisasi Otsu dan finder detection seperti biasa. Dua hal masih bisa salah: rasio foreground image yang sudah dibinarisasi bisa jadi degenerate (di bawah 5% atau di atas 95%, artinya efektif hanya satu class), atau finder detection masih bisa gagal karena alasan lain. Pada kedua kasus tersebut, pipeline me-rebinarise image grayscale dengan Sauvola dan menjalankan ulang finder detection. Baru ketika pass Sauvola juga gagal, decoder mengembalikan `ErrFinderNotFound`.
...
- Logika dispatch berupa blok `if η < ηMin { ... } else { ... }` kecil di dalam `decodeImage`, plus fallback reaktif `else if !found || !healthy { ... }`. Kita sengaja menjaganya sebagai kode lurus alih-alih strategy interface — hanya ada dua algoritma dan YAGNI berlaku.
