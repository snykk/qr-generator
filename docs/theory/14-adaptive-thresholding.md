...
**Stage 2 — reactive post-check.** If `η ≥ η_min` we run Otsu's binarisation and finder detection as usual. If `findFinders` succeeds, we trust the result and proceed. Otherwise we rebinarise the grayscale image with Sauvola and re-run finder detection; only when the Sauvola pass also fails does the decoder return `ErrFinderNotFound`. The original v0.3 plan also contemplated a foreground-ratio post-check (rejecting a binarisation when fewer than 5% or more than 95% of pixels are dark), but T5 benchmarking confirmed an empirical observation that retired the check: a degenerate single-class binarisation cannot produce valid 1:1:3:1:1 triples, so `findFinders` failure is already a strict superset of "ratio unhealthy", and skipping the extra `O(width * height)` ratio pass on the happy path keeps the Otsu fast path within ~1% of the v0.2 baseline.
...
- The dispatch logic is a small `if η < ηMin { ... } else { ... }` block inside `decodeImage`, plus a reactive `else if findFinders failed { ... }` fallback. We deliberately keep it as straight code instead of a strategy interface — there are only two algorithms and YAGNI applies.
