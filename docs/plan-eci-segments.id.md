...
5. **Penempatan setia-spec.** ECI header (mode indicator `0111` diikuti designator — istilah spec, terlepas dari nama longgar "ECI segment") dikeluarkan sekali di kepala data, sebelum deret mixed-mode segment, sesuai ISO/IEC 18004. Per klausul 15.2 sebuah ECI mengatur penafsiran semua data berikutnya; efek terlihatnya jatuh pada byte data hanya sebagai konsekuensi praktis, karena numeric dan alphanumeric meng-encode subset ASCII tetap yang sama di bawah tiap charset umum.
...
- **Decoder wajib menerima designator non-minimal.** Encoder mengeluarkan bentuk terpendek 1/2/3-codeword, tetapi ISO/IEC 18004 (Table 4, klausul 8.4.1.1) membolehkan nomor assignment kecil di-encode dalam bentuk lebih panjang — range per-kelas overlap dan semua mulai dari nol, yang terpendek hanya preferred. Jadi decoder membaca jumlah codeword dari bit prefix (`0`/`10`/`110`) dan menerima panjang apa pun, tak pernah mengasumsikan yang minimal. Plafon 999999 adalah cap enam-digit-desimal, bukan cap lebar-bit.
